"""Tests for the budget version ValueSet line operation service."""
...
from app.services import orcamento_valueset_linha_operacao_service as service_module


def _resumo(**kwargs) -> OrcamentoValuesetLinhaOperacaoResumo:
    base = {
        "id": 1,
    ...
    base.update(kwargs)
    return OrcamentoValuesetLinhaOperacaoResumo(**base)


class _FakeRepository:
    existing_links: list[OrcamentoValuesetLinhaOperacaoResumo] = []
    active_links: list[OrcamentoValuesetLinhaOperacaoResumo] = []
    by_id: OrcamentoValuesetLinhaOperacaoResumo | None = None
    ...
    def list_by_linha(
        ...
        self.__class__.requested_linha_id = orcamento_valueset_linha_id
    ...
    def create(self, **kwargs) -> OrcamentoValuesetLinhaOperacaoResumo:
        self.__class__.created_payload = kwargs
        self.__class__.created_payloads.append(kwargs)
        return _resumo(id=1, **kwargs)

    def update(self, **kwargs) -> OrcamentoValuesetLinhaOperacaoResumo:
    ...
    def delete_by_linha(self, orcamento_valueset_linha_id: int) -> int:
        self.__class__.deleted_linha_ids.append(orcamento_valueset_linha_id)
...
def _service(monkeypatch):
    ...
    return service_module.OrcamentoValuesetLinhaOperacaoService(session=session), session
...
def test_adicionar_normaliza_ordem_regra_e_tempos(monkeypatch) -> None:
    ...
    assert payload["orcamento_valueset_linha_id"] == 10
...
def test_editar_permite_a_propria_ligacao(monkeypatch) -> None:
    ...
    service.editar_operacao_da_linha(
        5,
        service_module.EditarOrcamentoValuesetLinhaOperacaoData(
            orcamento_valueset_linha_id=10,
            def_operacao_id=20,
            ordem=2,
        ),
