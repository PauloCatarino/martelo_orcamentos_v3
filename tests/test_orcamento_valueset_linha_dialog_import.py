...
def test_dialog_has_actions() -> None:
    ...
    calcular = inspect.getsource(OrcamentoValuesetLinhaDialog._calcular_preco_liquido)
...
def test_dialog_permite_nova_linha_e_gravar_como() -> None:
    ...
    gravar_como = inspect.getsource(OrcamentoValuesetLinhaDialog._validate_and_save_as)
    validar = inspect.getsource(OrcamentoValuesetLinhaDialog._validate_and_run)

    assert "Nova Linha ValueSet do Orçamento" in init_source
...
def test_nova_linha_orcamento_tem_chave_editavel_e_origem_local(monkeypatch) -> None:
    from PySide6.QtWidgets import QApplication

    from app.ui.dialogs import orcamento_valueset_linha_dialog as modulo
    ...
    guardado = []
    dialog = modulo.OrcamentoValuesetLinhaDialog(
        on_save=lambda dados: guardado.append(dados) or True
    )
    dialog.nome_opcao_input.setText("MDF local")
