...
def test_dropdowns_dentro_das_tabelas_ignoram_a_roda_do_rato() -> None:
    ...
    custeio = inspect.getsource(orcamento_item_custeio_page)
    assert "ComboSemScroll()" in custeio
    assert custeio.count("ComboSemScroll()") == 2  # Mat. default + PUR/LASER

    itens = inspect.getsource(orcamento_items_page)
    assert itens.count("ComboSemScroll()") == 2  # Produção + Custeio
