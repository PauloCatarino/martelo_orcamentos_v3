...
class OrcamentoItemValuesetLinhaOperacaoRepository:
    ...
    def list_active_by_linhas(
        ...
        """
        por_linha: dict[int, list[OrcamentoItemValuesetLinhaOperacaoResumo]] = {
        ...
        statement = (
            select(OrcamentoItemValuesetLinhaOperacao)
            .where(
                OrcamentoItemValuesetLinhaOperacao.orcamento_item_valueset_linha_id.in_(list(por_linha)),
                OrcamentoItemValuesetLinhaOperacao.ativo.is_(True),
            )
            .order_by(
                OrcamentoItemValuesetLinhaOperacao.orcamento_item_valueset_linha_id.asc(),
                OrcamentoItemValuesetLinhaOperacao.ordem.asc(),
                OrcamentoItemValuesetLinhaOperacao.id.asc(),
            )
        )
        for ligacao in self.session.execute(statement).scalars().all():
            por_linha[ligacao.orcamento_item_valueset_linha_id].append(
